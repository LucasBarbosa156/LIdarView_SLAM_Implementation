...
def estimate_transform(P_matched, Q_matched):
    p_bar, q_bar, P_centered, Q_centered = compute_centroids(P_matched, Q_matched)
    R = compute_rotation(P_centered, Q_centered)
    t = q_bar - np.dot(R, p_bar)
    return R, t

def find_correspondences(P, Q):
    dist_sq = np.sum(P**2, axis=1)[:, np.newaxis] + np.sum(Q**2, axis=1) - 2 * np.dot(P, Q.T)
...
def icp(P, Q, max_iterations=50, tolerance=1e-5):
    ...
    for i in range(max_iterations):
        ...
        if len(errors) > 1 and abs(errors[-2] - errors[-1]) < tolerance:
            print(f"  Convergiu na iteração {i}, erro final: {errors[-1]:.4f}m")
            break
        else:
            if i == max_iterations - 1:
                print(f"  Não convergiu após {max_iterations} iterações. Erro final: {errors[-1]:.4f}m")
